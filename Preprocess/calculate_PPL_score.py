...
def main(args):
    ...
    data_path = "/mnt/petrelfs/songmingyang/songmingyang/runs/llava/test/generations/{}".format(args.data_file)
    target_dir = "/mnt/petrelfs/songmingyang/songmingyang/runs/llava/test/add_ppl/{}".format(args.data_file)
    target_path = target_dir + "/{proc}.jsonl".format(proc = args.begin_index)
    ...
    def MultiLigual_Alighment_reward_fuction(tokenizer,rm_model,outputs,labels=None):
        ...
        l = language_dict[args.language]['full_name']
        langs = {
            'Swahili' :'swh_Latn',
            'Chinese' : "zho_Hans",
            "Bengali" : "ben_Beng",
